...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Times New Roman', Georgia, serif; }
        ...
        .encabezado { 
            background: linear-gradient(90deg,#00264d,#003366); 
            color: white; 
            padding: 22px 30px; 
            border-radius: 10px; 
            margin-bottom: 25px; 
        ...
        button { 
            background: linear-gradient(90deg,#cc9900,#e6ac00,#cc9900); 
            color: #00264d; 
            border: none; 
            padding: 14px 32px; 
            border-radius: 6px; 
        ...
        button:hover { 
            background: linear-gradient(90deg,#b38600,#cc9900); 
        }
    </style>
</head>
<body>
        ...
        <div class="tarjeta">
            ...
            <form method="POST" action="index.php?accion=bloque_editar&id=<?= $bloque['id_bloque'] ?>">
                ...
                <input type="text" 
                       id="nombre_bloque" 
                       name="nombre_bloque" 
                       value="<?= htmlspecialchars($bloque['nombre_bloque']) ?>" 
                       required>
                ...
                <textarea id="descripcion" 
                          name="descripcion" 
                          rows="3" 
                          placeholder="Breve descripción del bloque..."><?= htmlspecialchars($bloque['descripcion'] ?? '') ?></textarea>

                <button type="submit">💾 Actualizar Bloque Horario</button>
            </form>
        </div>
    </div>
</body>
