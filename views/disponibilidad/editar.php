...
<body>
    <div class="contenedor">
        <div class="encabezado">
            <h1>✏️ Editar Disponibilidad</h1>
            <a href="index.php?accion=disponibilidad_listar" class="volver-btn">← Volver</a>
        </div>
        <div class="tarjeta">
            <?php if ($error): ?><div class="mensaje-error"><?= htmlspecialchars($error) ?></div><?php endif; ?>
            <form method="POST" action="index.php?accion=disponibilidad_editar&id=<?= $d['id_disponibilidad'] ?>">
                <label for="id_tutor">Tutor:</label>
                <select name="id_tutor" id="id_tutor" required>
                    <?php while ($t = $tutores->fetch(PDO::FETCH_ASSOC)): ?>
                    <option value="<?= $t['id_tutor'] ?>" <?= $d['id_tutor']==$t['id_tutor']?'selected':'' ?>>
                ...
                <select name="dia_semana" id="dia_semana" required>
                    ...
                    foreach ($dias as $dia): ?>
                    <option value="<?= $dia ?>" <?= $d['dia_semana']==$dia?'selected':'' ?>><?= $dia ?></option>
                    <?php endforeach; ?>
                </select>
                <label for="hora_inicio">Hora Inicio:</label>
                <input type="time" id="hora_inicio" name="hora_inicio" value="<?= $d['hora_inicio'] ?>" required>
                <label for="hora_fin">Hora Fin:</label>
                <input type="time" id="hora_fin" name="hora_fin" value="<?= $d['hora_fin'] ?>" required>
                <label for="estado">Estado:</label>
                <select name="estado" id="estado">
                    <option value="disponible" <?= $d['estado']==='disponible'?'selected':'' ?>>Disponible</option>
                    <option value="ocupado" <?= $d['estado']==='ocupado'?'selected':'' ?>>Ocupado</option>
                </select>
