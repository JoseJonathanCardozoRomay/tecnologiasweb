...
<body>
    <div class="contenedor">
        ...
        <div class="tarjeta">
            <a href="index.php?accion=carrera_crear" class="btn btn-primario nuevo">+ Nueva Carrera</a>
            <?php if (($_GET['mensaje'] ?? '') === 'creado'): ?><div class="mensaje mensaje-exito">✅ Carrera creada correctamente.</div><?php endif; ?>
            <?php if (($_GET['mensaje'] ?? '') === 'actualizado'): ?><div class="mensaje mensaje-exito">✅ Carrera actualizada correctamente.</div><?php endif; ?>
            <?php if (($_GET['mensaje'] ?? '') === 'eliminado'): ?><div class="mensaje mensaje-exito">✅ Carrera eliminada correctamente.</div><?php endif; ?>
            <table>
                <tr><th>ID</th><th>Nombre de la Carrera</th><th>Acciones</th></tr>
                <?php while ($carrera = $carreras->fetch(PDO::FETCH_ASSOC)): ?>
                <tr>
                    <td><?= $carrera['id_carrera'] ?></td>
                    <td><?= htmlspecialchars($carrera['nombre_carrera']) ?></td>
                    <td>
                        <a href="index.php?accion=carrera_editar&id=<?= $carrera['id_carrera'] ?>" class="btn btn-aviso">Editar</a>
                        <a href="index.php?accion=carrera_eliminar&id=<?= $carrera['id_carrera'] ?>" class="btn btn-peligro" onclick="return confirm('¿Eliminar esta carrera?')">Eliminar</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            </table>
