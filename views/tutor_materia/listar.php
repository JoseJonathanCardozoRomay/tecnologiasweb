...
<head>
    <meta charset="UTF-8">
    <title>Asignación Tutor-Materia</title>
    <style>
...
<body>
    <div class="contenedor">
        ...
        <div class="tarjeta">
            <a href="index.php?accion=asignar_crear" class="btn btn-primario nuevo">+ Nueva Asignación</a>
            <?php if (($mensaje ?? '') === 'creado'): ?><div class="mensaje mensaje-exito">✅ Asignación registrada.</div><?php endif; ?>
            <?php if (($mensaje ?? '') === 'eliminado'): ?><div class="mensaje mensaje-exito">✅ Asignación eliminada.</div><?php endif; ?>
            <table>
                <tr><th>ID</th><th>Tutor</th><th>Materia</th><th>Acciones</th></tr>
                <?php while ($a = $asignaciones->fetch(PDO::FETCH_ASSOC)): ?>
                <tr>
                    <td><?= $a['id_asignacion'] ?></td>
                    <td><?= htmlspecialchars($a['tutor_nombre']) ?></td>
                    <td><?= htmlspecialchars($a['materia_nombre']) ?></td>
                    <td>
                        <a href="index.php?accion=asignar_eliminar&id=<?= $a['id_asignacion'] ?>" class="btn btn-peligro" onclick="return confirm('¿Quitar asignación?')">Eliminar</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            </table>
