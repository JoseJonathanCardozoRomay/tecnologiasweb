...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Times New Roman', Georgia, serif; }
        ...
        select { width: 100%; padding: 12px 15px; border: 1px solid #99b3cc; border-radius: 6px; font-size: 15px; background: #fafcff; }
        select:focus { outline: none; border-color: #cc9900; box-shadow: 0 0 0 3px rgba(204,153,0,0.2); }
...
<body>
    <div class="contenedor">
        <div class="encabezado">
            ...
            <a href="index.php?accion=asignar_listar" class="volver-btn">← Volver</a>
        </div>
        <div class="tarjeta">
            <?php if ($error): ?><div class="mensaje-error"><?= htmlspecialchars($error) ?></div><?php endif; ?>
            <form method="POST" action="index.php?accion=asignar_editar&id=<?= $asig['id_asignacion'] ?>">
                <label for="id_tutor">Tutor:</label>
                <select name="id_tutor" id="id_tutor" required>
                    <?php while ($t = $tutores->fetch(PDO::FETCH_ASSOC)): ?>
                    <option value="<?= $t['id_tutor'] ?>" <?= $asig['id_tutor']==$t['id_tutor']?'selected':'' ?>>
                ...
                <select name="id_materia" id="id_materia" required>
                    ...
                    <option value="<?= $m['id_materia'] ?>" <?= $asig['id_materia']==$m['id_materia']?'selected':'' ?>>
