...
<div class="d-flex flex-column flex-lg-row justify-content-between gap-3 mb-4">
    <div>
        ...
        <h1 class="page-title mb-1">Estudiantes y solicitudes</h1>
...
<div class="row g-4">
        ...
        <div class="card card-custom overflow-hidden">
            <div class="p-4 border-bottom">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge text-bg-light border fs-6"><i class="bi bi-people me-1"></i><?=count(array_filter($estudiantes, fn($x) => in_array($x['estado'], ['aprobada','inscrito','asistio'], true)))?></span>
                </div>
            </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <?php foreach($estudiantes as $e): ?>
                        <tr>
                            ...
                            <td><?=e($e['semestre'])?></td>
                            <td><span class="badge text-bg-<?=match($e['estado']){'pendiente'=>'warning','aprobada'=>'primary','inscrito'=>'primary','asistio'=>'success','no_asistio'=>'danger','retirado'=>'secondary',default=>'light'}?>"><?=e(estadoEtiqueta((string)$e['estado']))?></span></td>
                            <td class="text-end">
                                <?php if(esAdministrador() && $sesion['estado']==='programada' && $e['estado']==='pendiente'): ?>
                                    <form method="post" action="tutorias_grupales_estudiante_admin_accion.php" onsubmit="return confirm('¿Retirar a este estudiante de la sesión?');">
                                        ...
                                        <button class="btn btn-sm btn-primary" name="accion" value="aprobar">Aprobar</button>
                                    </form>
                                <?php elseif(esAdministrador() && $sesion['estado']==='programada' && in_array($e['estado'],['aprobada','inscrito'],true)): ?>
                                    <form method="post" action="tutorias_grupales_estudiante_admin_accion.php" onsubmit="return confirm('¿Retirar a este estudiante de la sesión?');">
                                        <?=csrfField()?>
                                        <input type="hidden" name="id_tutoria_grupal" value="<?=e($id)?>">
                                        <input type="hidden" name="id_estudiante" value="<?=e($e['id_estudiante'])?>">
                                ...
                                <?php elseif(esTutor() && in_array($sesion['estado'],['en_curso','realizada'],true) && in_array($e['estado'],['aprobada','inscrito','asistio'],true)): ?>
    ...
    <div class="col-xl-4">
        <div class="card card-custom p-4">
            ...
            <p class="small text-muted">Solo aparecen estudiantes activos que no tengan una solicitud o inscripción vigente en esta sesión.</p>
            <form method="post" action="tutorias_grupales_estudiante_admin_accion.php" class="d-grid gap-3">
                <?=csrfField()?>
                <input type="hidden" name="id_tutoria_grupal" value="<?=e($id)?>">
                <select name="id_estudiante" class="form-select tom-select" required>
                    <option value="">Seleccionar estudiante</option>
                    <?php foreach($disponibles as $d): ?>
                        <option value="<?=e($d['id_estudiante'])?>"><?=e($d['apellido'].' '.$d['nombre'].' — '.$d['registro_universitario'])?></option>
                    <?php endforeach; ?>
                </select>
                <button class="btn btn-primary" name="accion" value="inscribir"><i class="bi bi-person-plus me-1"></i>Inscribir y aprobar</button>
            </form>
        </div>
    </div>
    <?php endif; ?>
