    ...
    <div class="contenedor">
        ...
        <div class="tarjeta">
            ...
            <form method="POST" action="index.php?accion=periodo_editar&id=<?= $periodo['id_periodo'] ?>">
                ...
                <input type="text" id="codigo" name="codigo" value="<?= htmlspecialchars($periodo['codigo']) ?>" required>
                <label for="nombre">Nombre:</label>
                <input type="text" id="nombre" name="nombre" value="<?= htmlspecialchars($periodo['nombre']) ?>" required>
                <label for="fecha_inicio">Fecha Inicio:</label>
                <input type="date" id="fecha_inicio" name="fecha_inicio" value="<?= $periodo['fecha_inicio'] ?>" required>
                <label for="fecha_fin">Fecha Fin:</label>
                <input type="date" id="fecha_fin" name="fecha_fin" value="<?= $periodo['fecha_fin'] ?>" required>
                <label for="activo">Estado:</label>
                <select name="activo" id="activo">
                    <option value="1" <?= $periodo['activo']?'selected':'' ?>>Activo</option>
                    <option value="0" <?= !$periodo['activo']?'selected':'' ?>>Inactivo</option>
                </select>
