...
<body>
    <div class="contenedor">
        ...
        <div class="tarjeta">
            <a href="index.php?accion=materia_crear" class="btn btn-primario nuevo">+ Nueva Materia</a>
            <?php if (($_GET['mensaje'] ?? '') === 'creado'): ?><div class="mensaje mensaje-exito">✅ Materia creada correctamente.</div><?php endif; ?>
            <?php if (($_GET['mensaje'] ?? '') === 'actualizado'): ?><div class="mensaje mensaje-exito">✅ Materia actualizada correctamente.</div><?php endif; ?>
            <?php if (($_GET['mensaje'] ?? '') === 'eliminado'): ?><div class="mensaje mensaje-exito">✅ Materia eliminada correctamente.</div><?php endif; ?>
            <table>
                <tr><th>ID</th><th>Nombre de la Materia</th><th>Carrera</th><th>Acciones</th></tr>
                <?php while ($materia = $materias->fetch(PDO::FETCH_ASSOC)): ?>
                <tr>
                    <td><?= $materia['id_materia'] ?></td>
                    <td><?= htmlspecialchars($materia['nombre_materia']) ?></td>
                    <td><?= htmlspecialchars($materia['nombre_carrera'] ?? 'General') ?></td>
                    <td>
                        <a href="index.php?accion=materia_editar&id=<?= $materia['id_materia'] ?>" class="btn btn-aviso">Editar</a>
                        <a href="index.php?accion=materia_eliminar&id=<?= $materia['id_materia'] ?>" class="btn btn-peligro" onclick="return confirm('¿Eliminar esta materia?')">Eliminar</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            </table>
