...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validarTokenCsrf()) {
        ...
        $modeloCohorte->existeCodigo(
            $codigo,
            $idCohorte
        )
    ) {
        $error = 'Ya existe otra cohorte con ese código.';
    } else {
        try {
            $modeloCohorte->actualizar(
                $idCohorte,
                $codigo,
                $nombre,
                $fechaInicio,
            ...
            header(
                'Location: cohortes_listar.php?estado=actualizado'
            );
            exit;
        } catch (PDOException $e) {
            $error = 'No fue posible actualizar la cohorte.';
        }
    }
}

// Datos utilizados por la vista
$tituloPagina = 'Editar cohorte';
$rutaBase = '../';

require_once __DIR__
    . '/../views/cohortes/editar.php';