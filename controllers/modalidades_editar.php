...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validarTokenCsrf()) {
        ...
        if (
            ...
            $modeloModalidad->existeCodigo(
                $codigo,
                $idModalidad
            )
        ) {
            $error = 'Ya existe otra modalidad con ese código.';
        } elseif (
            $modeloModalidad->existeNombre(
                $nombre,
                $idModalidad
            )
        ) {
            $error = 'Ya existe otra modalidad con ese nombre.';
        } else {
            try {
                $modeloModalidad->actualizar(
                    $idModalidad,
                    $codigo,
                    $nombre,
                    $descripcion !== ''
                ...
                header(
                    'Location: modalidades_listar.php?estado=actualizado'
                );
                exit;
            } catch (PDOException $e) {
                $error = 'No fue posible actualizar la modalidad.';
            }
        }
    }
}
...
$tituloPagina = 'Editar modalidad';
$rutaBase = '../';

require_once __DIR__ . '/../views/modalidades/editar.php';